...
export function handleDataCenterChange(client) {
  ...
  document.querySelector("#data-center-name").value =
    dataCenterSelect.options[dataCenterSelect.selectedIndex].innerText;
  ...
  if (selectedDataCenter) {
    ...
    fetch(url)
      ...
      .then((html) => {
        ...
        document.getElementById("form-title").innerText = `New ${
          dataCenterSelect.options[dataCenterSelect.selectedIndex].innerText
        } Support Ticket`;
        ...
        const updateMessageBody = () => {
          ...
          const zendeskTicketId =
            zendeskTicketIdInput.value || "{zendeskTicketId}";
          ...
          if (template) {
            const templateVars = {
              ...
              logs,
              zendeskTicketId,
              "{ticket.submitter.name}": dataCentersWithoutAPI.includes(
                dataCenter
              )
                ? "{{ticket.submitter.name}}"
                : document.querySelector("#zendesk-username").value,
            };
          ...
          if (document.querySelector("#subject")) {
            if (
              issueCategory !== "Custom Issue" &&
              issueCategory !== "{issueCategory}"
            ) {
              document.querySelector(
                "#subject"
              ).value = `${issueCategory} - ${ipAddressString} - ZD-${zendeskTicketId}`;
            }
          }
        };
        ...
        const showOrHideElements = (issueCategory) => {
          ...
          const ipAddressContainer = document.querySelector(
            "#ip-address-container"
          );
          const osVersionContainer = document.querySelector(
        ...
        document
          .querySelector("#issue-category")
          .addEventListener("change", function () {
            const selectedIssueCategory = this.value;
            if (
              !selectedIssueCategory ||
              selectedIssueCategory === "Select Issue Category"
            ) {
            ...
            if (selectedIssueCategory === "Custom Issue") {
              ...
              const templateVars = getTemplateVars(
                messageTemplates[selectedIssueCategory]
              );
        ...
        if (issueCategorySelect.value) {
          const templateVars = getTemplateVars(
            messageTemplates[issueCategorySelect.value]
          );
          showOrHideElements(templateVars);
        }
      })
...
export function addFormEventListeners() {
  ...
  ipAddressTextarea.addEventListener("input", function () {
    const templateVars = document.querySelector("#issue-category").value
      ? getTemplateVars(
          messageTemplates[document.querySelector("#issue-category").value]
        )
      : [];
...
function displayFileNames(files, fileList, fileInput) {
  ...
  if (fileList) {
    ...
    const isValid = validateFileCount(
      selectedDataCenter,
      allFiles.length,
      fileInput
    );
