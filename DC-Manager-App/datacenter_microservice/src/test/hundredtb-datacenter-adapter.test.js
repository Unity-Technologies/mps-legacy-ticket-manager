...
const {
  ...
  invalidApiKey,
} = require("./test-data/hundredtb-response.js");
...
const hundredtbConfigNoApiKey = {
  baseUrl: "http://localhost", // Replace with a mock URL for testing
  apiKey: null, // Replace with a mock API key for testing
};
...
describe("Create 100TB Ticket", () => {
  ...
  test("creates a ticket with valid data (mocked success)", async () => {
    ...
    const createdTicket = await hundredtbAdapter.createTicket(ticketData);

    expect(createdTicket).toEqual({ id: "2981" }); // Assert the returned ticket data
    expect(axios.post).toHaveBeenCalledWith(
      `${hundredtbConfig.baseUrl}/tickets`,
...
describe("Get 100TB Ticket", () => {
  test("retrieves a ticket by ID", async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith(
      `${hundredtbConfig.baseUrl}/tickets/${ticketId}`,
      // Validate request body structure
      {
        headers: {
          // Validate headers including X-Api-Token
